...
class FixedDimGraphEnvironment(gym.Env):
    ...
    def __init__(self, hawkes_model, all_entities, max_entity_num=256, max_steps=100):
        """
        初始化环境
        ...
        # 确保实体数量等于max_entity_num
        if len(all_entities) < max_entity_num:
            raise ValueError(f"实体数量({len(all_entities)})小于指定的固定维度({max_entity_num})，请先生成足够的实体")
